...
class Actor(nn.Module):
    def __init__(self, s_dim, n_actions, fc1_dim=64, fc2_dim=64):
        ...
        self.ln1 = nn.LayerNorm(fc1_dim)  # Helps stabilize training
        # Second hidden layer
        self.fc2 = nn.Linear(fc1_dim, fc2_dim)
        # Output layer that maps to action space
        self.out = nn.Linear(fc2_dim, n_actions)

    def forward(self, state):
        x = F.relu(self.ln1(self.fc1(state)))
        x = F.relu(self.fc2(x))
        return T.tanh(self.out(x))  # Output bounded between -1 and 1
...
class MATD3:
    def __init__(self, s_dim, a_dim, num_agents, lr_actor=1e-3, lr_critic=1e-3,
        ...
        # Create actor networks and their target networks for each agent
        ...
        # Centralized critic networks (shared across all agents)
        ...
        # Initialize target networks with same weights as the main networks
    ...
    def choose_action(self, local_state, agent_idx, explore=False):
        # Get action from actor network for a given agent
    ...
    def learn(self):
        ...
        with T.no_grad():
            ...
            for i in range(self.num_agents):
                ...
                next_action = (next_action + 1) / 2  # Scale to [0,1]
        ...
        if self.learn_step % self.policy_delay == 0:
            for i in range(self.num_agents):
                ...
                pred_action = (pred_action + 1) / 2  # Scale to [0,1]

                # Replace action of current agent only
    ...
    def _get_local_state_batch(self, states, agent_idx):
        # Extracts local observation for a given agent from global state
        start = agent_idx * 3
