...
LR_A = 0.0001892743991686467
LR_C = 0.003178430756432114
GAMMA = 0.9400000000000001
TAU = 0.00477758577637705
...
for i in range(MAX_EPISODES):
    ...
    print(f"[Episode {i}] SR -> MADDPG: {sr/MAX_EP_STEPS:.4f} - EE -> MADDPG: {ee/MAX_EP_STEPS:.4f} - EH -> MADDPG: {eh}")
...
fig, ax = plt.subplots()
ax.plot(eh_rewardall_maddpg, "s-", label='MADDPG', linewidth=0.75, color='green')
ax.set_xlabel("Episodes")
...
fig.savefig(f"maddpg_avg_eh_{diversity_mode}.eps", format="eps", bbox_inches="tight")
fig.savefig(f"maddpg_avg_eh_{diversity_mode}.png", format="png", bbox_inches="tight")
plt.show()
