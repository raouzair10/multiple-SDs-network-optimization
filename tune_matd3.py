...
def objective(trial):
    ...
    policy_noise = trial.suggest_float('policy_noise', 0.1, 0.5, step=0.1)
    noise_clip = trial.suggest_float('noise_clip', 0.3, 0.7, step=0.1)
    ...
    # Create MATD3 agent with suggested hyperparameters
    # Note: MATD3 constructor expects global dimensions (s_dim=7, a_dim=2) for replay buffer and critic
    # The Actor networks inside MATD3 are hardcoded to use local dimensions (4, 1)
    matd3_agent = MATD3(s_dim, a_dim, num_SDs, lr_actor, lr_critic, gamma, tau, 
                        memory_capacity, batch_size, policy_noise, noise_clip, policy_delay)
    ...
    for i in range(MAX_EPISODES):
        ...
        for j in range(MAX_EP_STEPS):
            sd = choose_SD(s_matd3)
            s_local = np.append(s_matd3[0][sd * 3:(sd + 1) * 3], s_matd3[0][-1])
            a = matd3_agent.choose_action(s_local, sd, explore=True)
            a = np.clip(a[0], 0, 1)
            action_vec = np.zeros(a_dim, dtype=np.float32)
            action_vec[sd] = a

            r, s_next, _, eep, srp = env.step(sd, a, s_matd3 / state_am, j+i)
            s_next *= state_am
            matd3_agent.remember(s_matd3[0], action_vec, r, s_next[0])
            matd3_agent.learn()
            s_matd3 = s_next
...
def main():
    ...
    # Save results
    with open('matd3_best_params.txt', 'w') as f:
