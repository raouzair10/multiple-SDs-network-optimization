...
def objective(trial):
    ...
    # Create MADDPG agent with suggested hyperparameters
    # Note: MADDPG constructor expects global dimensions (s_dim=7, a_dim=2) for replay buffer and critic
    # The Actor networks inside MADDPG are hardcoded to use local dimensions (4, 1)
    maddpg_agent = MADDPG(s_dim, a_dim, num_SDs, lr_actor, lr_critic, gamma, tau, memory_capacity, batch_size)
...
def main():
    ...
    # Save results
    with open('maddpg_best_params.txt', 'w') as f:
